 ...
 *
 * A decoder thread demuxes the .m4v with FFmpeg, decoding H.264 into a ring of
 * tightly-packed YUV420 frames and streaming the AAC audio through an OpenAL
 * queue. The render side runs inside the eglSwapBuffers hook: it picks the ring
 * frame due by the audio clock, uploads it into three luminance textures and
 * draws a letterboxed fullscreen quad with a YUV->RGB shader.
 *
...
// ring depth must exceed the longest run of video packets the muxer
// interleaves between audio packets, else the decoder blocks on video before
// feeding the audio queue and playback deadlocks
...
// ---------------------------------------------------------------------------
// subtitles: the game pushes one line per call through
// GamePlatformServices.setMovieText (empty string clears); Android drew it in a
// TextView above the video view
// ---------------------------------------------------------------------------
...
void movie_set_text_scale(int scale) {
  // the Java TextView used this for point size; the GL overlay sizes glyphs
  // from the screen height instead, so the value is ignored
...
static double movie_clock(void) {
  if (mp.has_audio) {
    // hold playback until the audio queue is primed; running video on another
    // clock first and switching to the audio clock later would stall the
    // pipeline (already-shown frames would be ahead of the audio)
    if (!mp.audio_primed)
      return -1.0;
    ALint off = 0;
    mtx_lock(&mp.lock);
...
// second. Runs outside movie_render's guarded section, so it saves/restores all
// the GL state it touches itself.
// ---------------------------------------------------------------------------
...
static void movie_render(void) {
  ...
  // failed: the decoder blocks on a full ring, so playback (and the game
  // waiting on it) only progresses if we keep consuming
...
unsigned int eglSwapBuffersHook(void *display, void *surface) {
  ...
  if (config.show_fps)
    fps_render();
  game_swaps++;
  if (game_swaps <= 3 || (game_swaps % 120) == 0) {
    // sample the back buffer: all-zero means the game drew a black frame
    // (content), non-black means it has content (presentation issue instead)
    unsigned char c[4] = {0}, q[4] = {0}, e[4] = {0};
    glReadPixels(screen_width / 2, screen_height / 2, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, c);
    glReadPixels(screen_width / 4, screen_height / 4, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, q);
    glReadPixels(screen_width / 8, screen_height / 8, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, e);
    debugPrintf("egl swap #%u  px(mid)=%02x%02x%02x px(q)=%02x%02x%02x px(e)=%02x%02x%02x\n",
        game_swaps, c[0], c[1], c[2], q[0], q[1], q[2], e[0], e[1], e[2]);
  }
  return eglSwapBuffers((EGLDisplay)display, (EGLSurface)surface);
}

// the game stops rendering while waiting for a movie (Android covered the
// screen with a Java video view), so nothing calls eglSwapBuffers and playback
// would stall. Called every frame: when a movie is active and the game didn't
// present since the last tick, we render and present it ourselves. The swap
// also blocks on vsync, pacing the otherwise idle wait loop.
...
// ---------------------------------------------------------------------------
// Read the movie out of the game's WAD via the engine's cFile (which resolves
// through LogicalFS_OpenBundleFile -> data_main.wad, with a physical-file
// fallback). The .m4v movies are packed in the WAD on this build, not loose, so
// a custom ffmpeg AVIO backed by cFile decodes them in place.
// cFile layout: +0 inner file ptr, +8 cached size.
// ---------------------------------------------------------------------------
...
// mapped: so_flush_caches() frees the .so image (and symtab) right after
// so_finalize(), so a later so_try_find_addr_rx() would touch freed memory.
// wad_cfile_resolve caches its result, so the runtime path never re-resolves.
...
static int wad_avio_read(void *opaque, uint8_t *buf, int size) {
  // cFile::read is all-or-nothing: a short read (the file tail, or a seek-to-EOF
  // read like the MP4 'moov' atom) returns 0, which FFmpeg sees as a spurious
  // EOF and rejects the file. Clamp to the bytes remaining so the read is exact.
...
int movie_play(const char *path, int skippable) {
  ...
  // Prefer the engine WAD (the .m4v movies are packed inside data_main.wad on
  // this build, not loose); fall back to a loose file on the filesystem.
